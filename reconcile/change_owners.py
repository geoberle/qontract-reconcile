...
class BundleFileChange:
    ...
    def diff(self) -> Optional[DeepDiff]:
        if self.old and self.new:
            # todo (goberlec) - find correct deepdiff settings to detect deeply nested array changes while ignoring order
            return DeepDiff(self.old, self.new, ignore_order=True)
        else:
            return None
...
def extract_datafile_context_from_bundle_change(
    ...
    for c in change_type.changes:
        if c.change_schema == bundle_change.fileref.schema and c.context:
            context_selector = parse(c.context.selector)
...
def _parse_bundle_changes(bundle_changes) -> list[BundleFileChange]:
    change_list = [BundleFileChange(**c) for c in bundle_changes["datafiles"].values()]
    change_list.extend(
        [BundleFileChange(**c) for c in bundle_changes["resources"].values()]
    )
    return change_list
...
def build_change_type_contexts_from_self_service_roles(
    ...
    change_type_contexts: list[ChangeTypeContext] = []
    for bc in bundle_changes:
        for ct in change_types.values():
            context_files = extract_datafile_context_from_bundle_change(bc, ct)
            for cf in context_files:
                # if the context file is bound with the change type in
                # a role, build a changetypecontext
                for role in role_lookup[(cf.file_type, cf.path, ct.name)]:
...
def run(dry_run: bool, comparison_sha: str):
    ...
    contexts = build_change_type_contexts_from_self_service_roles(
        ...
        change_types={ct.name: ct for ct in fetch_change_types()},
