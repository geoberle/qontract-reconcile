...
def compile_jinja2_template(jinja_env, env, body):
...
def process_jinja2_template(body, vars=None, env=None, settings=None):
    ...
    try:
        ...
        template = compile_jinja2_template(jinja_env, env, body)
...
def early_exit_monkey_patch():
    ...
    orig_compile_jinja2_template = compile_jinja2_template

    jinja_template_cache = {}

    def cached_compile_jinja2_template(jinja_env, env, template_body):
        cache_key = f"{env},{template_body}"
        if cache_key not in jinja_template_cache:
            jinja_template_cache[cache_key] = jinja_env.from_string(template_body)
        return jinja_template_cache[cache_key]
    ...
    try:
        yield early_exit_monkey_patch_assign(
            ...
            cached_compile_jinja2_template,
        )
    finally:
...
def early_exit_monkey_patch_assign(
    ...
    sys.modules[__name__].compile_jinja2_template = compile_jinja2_template
